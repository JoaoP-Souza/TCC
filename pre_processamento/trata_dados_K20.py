...
def process_file(filename, prefix):
    ...
    reshaped_data = data.reshape(10000, 20)

    # Criar um DataFrame com as 10 colunas, nomeadas com o prefixo fornecido
    new_df = pd.DataFrame(reshaped_data, columns=[f'{prefix}{i+1}' for i in range(20)])

    return new_df

# Processar os arquivos Pos_X e Pos_Y
pos_x_df = process_file('datasets_txt/K20/Pos_x_K20.txt', 'Pos_X')
pos_y_df = process_file('datasets_txt/K20/Pos_y_K20.txt', 'Pos_Y')

# Processar Best_X e Best_Y como colunas únicas, removendo espaços ou vírgulas extras
best_x = pd.read_csv('datasets_txt/K20/Best_X_K20.txt', delimiter='\t', header=None, names=['Best_X'])
best_y = pd.read_csv('datasets_txt/K20/Best_Y_K20.txt', delimiter='\t', header=None, names=['Best_Y'])
...
combined_df.to_csv('df_final_K20.csv', index=False)
