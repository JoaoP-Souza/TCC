...
def process_file(filename, prefix):
    ...
    reshaped_data = data.reshape(10000, 40)

    # Criar um DataFrame com as 10 colunas, nomeadas com o prefixo fornecido
    new_df = pd.DataFrame(reshaped_data, columns=[f'{prefix}{i+1}' for i in range(40)])

    return new_df

# Processar os arquivos Pos_X e Pos_Y
pos_x_df = process_file('datasets_txt/K40/Pos_x_K40.txt', 'Pos_X')
pos_y_df = process_file('datasets_txt/K40/Pos_y_K40.txt', 'Pos_Y')

# Processar Best_X e Best_Y como colunas únicas, removendo espaços ou vírgulas extras
best_x = pd.read_csv('datasets_txt/K40/Best_X_K40.txt', delimiter='\t', header=None, names=['Best_X'])
best_y = pd.read_csv('datasets_txt/K40/Best_Y_K40.txt', delimiter='\t', header=None, names=['Best_Y'])
...
combined_df.to_csv('df_final_K40.csv', index=False)
